...
app.get("/filter", (req, res) => {
  const par = req.query.type;
  const foundJoke = jokes.filter((joke) => joke.jokeType == par);
  res.json(foundJoke);
})
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if(searchIndex > -1){
    ...
    res
      .status(400)
      .json({error: `Joke with id: ${id} does not exist.`});
  }
});
//8. DELETE All jokes
app.delete("/all", (req, res) => {
  ...
  if(userKey == masterKey){
    ...
    res
      .status(400)
      .json({error: "Sorry you are not authorized to delete."})
  }
})
app.listen(port, () => {
