...
app.post("/submit", (req, res) => {
  ...
  const randomNumber = Math.floor(Math.random() * adj.length);
  const fName = adj[randomNumber];

  const randomNumber2 = Math.floor(Math.random() * noun.length);
  const lName = noun[randomNumber2];

  const bName = fName + " " + lName;
  res.render("index.ejs", {bandName: bName});
